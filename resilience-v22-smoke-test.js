...
function hasCurrentModule(source, file, minimum = 22) {
  const matches = [...source.matchAll(new RegExp(`/${file.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\?v=(\\d+)`, 'g'))];
...
if (releaseNumber < 22) throw new Error(`Vercel release header must report v22 or newer, got ${releaseHeader || 'missing'}.`);

...
export async function enrichBellaSemanticMemoryV30(req) {
  const body = req?.body;
  const originalMessage = clean(body?.message, 1000);
  const queryText = clean(req?.bellaGoalThreadV31?.retrievalQuery || originalMessage, 1000);
  if (!body || typeof body !== "object" || !shouldRetrieve(queryText)) return { active: false, reason: "skip" };
  ...
  if (!rows.length) return { active: true, indexed: 0, retrieved: 0, version: "v30", threadExpanded: queryText !== originalMessage };
  ...
  if (apiKey) {
    try {
      const vectors = await createEmbeddings([queryText, ...stale.map(row => row.memory_text)], apiKey);
  ...
  let matches = [];
  try { matches = await hybridSearch(token, queryText, queryEmbedding); }
  catch { return { active: true, indexed, retrieved: 0, reason: "search_unavailable", version: "v30", threadExpanded: queryText !== originalMessage }; }

  const diverse = diversifyRows(matches);
  const retrieved = mergeRetrievedMemory(body, diverse);
  if (body.memoryContextV30) body.memoryContextV30.threadExpanded = queryText !== originalMessage;
  ...
  return {
    ...
    confidenceAware: true,
    contradictionAware: true,
    threadExpanded: queryText !== originalMessage,
    version: "v30"
  };
}

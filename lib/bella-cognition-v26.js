...
function classifyTask(message, history = []) {
  ...
  const social = /^(?:هلا|هلو|هاي|سلام|شلونج|شلونك|احبج|احبك|اشتقتلج|موا|هههه|😂|🤣|تمام|اوكي|زين|اي|لا)\b/.test(text) && words <= 14;
  const transform = /^(?:ترجم|اكتب|صيغ|صيغي|عدل|عدلي|اختصر|لخص|صحح|صححي|قول|قولي)\b/.test(text);
  ...
  const followup = words <= 12 && Array.isArray(history) && history.length > 0 && /^(?:ليش|شلون|كيف|وهذا|وهذي|هذا|هذي|نفسه|نفسها|كمل|كملي|وبعدين|يعني|طيب|زين|اي|لا|قصدي)\b/.test(text);
  ...
  return { kind, words, signals: unique(signals), sensitive, correction, followup, multiStep, technical, analytical, social, transform };
}
...
function chooseModel(base, task, ambiguity) {
  ...
  if (reasoning.tier === "deep" || score >= 7 || task.multiStep || (task.technical && task.analytical) || (task.sensitive && task.analytical)) {
    return { tier: "sol", id: "gpt-5.6-sol", fallback: "gpt-5-mini", reason: "deep-complexity" };
  }
  if (reasoning.tier === "focused" || task.followup || task.correction || ambiguity.tier !== "low" || freshness.tier === "required") {
...
function verificationPlan(base, task, ambiguity) {
  ...
  if (ambiguity.useHistoryToResolve || task.correction) return { mode: "context-check", level: "medium", reasons: unique([ambiguity.useHistoryToResolve ? "resolve-reference" : "", task.correction ? "correction" : ""]) };
  return { mode: "light", level: "low", reasons: [] };
}

function contextPlan(base, task) {
  const deep = base.reasoning?.tier === "deep" || task.multiStep;
  const focused = base.reasoning?.tier === "focused" || task.followup || task.correction;
  return {

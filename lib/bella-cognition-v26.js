...
function classifyTask(message, history = []) {
  ...
  // Corrections have priority over casual prefixes such as "لا". Otherwise a message
  // like "لا مو جذي، قصدي..." can be misclassified as social and skip stronger reasoning.
  const correction = /(?:فهمتي غلط|غلط|قصدي|لا اقصد|لا مو|مو جذي|صحح|تصحيح)/.test(text);

  // JavaScript \b is ASCII-centric and misses Arabic word endings. Accept repeated
  // laughter/emoji as social turns and use explicit whitespace/end boundaries.
  const social = /^(?:(?:هلا|هلو|هاي|سلام|شلونج|شلونك|احبج|احبك|اشتقتلج|موا|تمام|اوكي|زين|اي|لا)(?:\s|$)|ه{3,}(?:\s|$)|[😂🤣]+(?:\s|$))/.test(text) && words <= 14 && !correction;
  ...
  // Explicit rewrite/translate/write commands outrank explanatory words nested inside
  // the requested text (e.g. "قول لي شلون اكتبها رسمي").
...
export function bellaCognitionInstructionV26(plan = {}) {
  ...
  if (typeof plan.metacognitiveInstruction === "string" && plan.metacognitiveInstruction.trim()) {
    lines.push(plan.metacognitiveInstruction.trim().slice(0, 5000));
  }
  return lines.join("\n");
}

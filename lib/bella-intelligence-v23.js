...
export function bellaFreshnessRouteV23(message) {
  ...
  const explicitCurrent = /(?:اليوم|الحين|الان|توه|توها|باجر|هالاسبوع|هذا الاسبوع|اخر|احدث|جديد|مباشر|latest|today|now|current|tonight|this week)/.test(text);
  ...
  const currentRole = /(?:منو|من هو|من هي|who is)\s+(?:الرئيس|رئيس|الامير|الملك|الوزير|المدير التنفيذي|ceo|رئيس الوزراء)/.test(text);

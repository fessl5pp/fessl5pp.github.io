...
export function bellaFreshnessRouteV23(message) {
  ...
  const latestUpdate = /(?:احدث|اخر|latest|newest)\s+(?:تحديث|update|release)|(?:تحديث|update|release)\s+(?:الحالي|الجديد|احدث|اخر|latest|current)/.test(text);
  ...
  add(latestUpdate, 5, "latest-update");
...
export function bellaReasoningRouteV23(message, history = []) {
  ...
  // Arabic word boundaries cannot rely on JavaScript \b. Keep explicit end/space
  // boundaries and accept repeated laughter/emoji as casual social turns.
  const social = /^(?:(?:هلا|هلو|هاي|سلام|شلونج|شلونك|احبج|احبك|اشتقتلج|موا|تمام|اوكي|زين|اي|لا)(?:\s|$)|ه{3,}(?:\s|$)|[😂🤣]+(?:\s|$))/.test(text) && words <= 12;
  const directSimple = /^(?:ترجم|اكتب|قول|قولي|شنو معنى|وش معنى)(?:\s|$)/.test(text) && words <= 18;

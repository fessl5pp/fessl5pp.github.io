...
import { bellaCognitionInstructionV26 } from "./bella-cognition-v26.js";
...
function applyCognitiveBrainV26(body, plan) {
  if (!plan || plan.release !== "v26" || !plan.model?.id) return body;

  body.model = plan.model.id;
  const effort = ["low", "medium", "high"].includes(plan.reasoning?.effort) ? plan.reasoning.effort : "low";
  const verbosity = ["low", "medium", "high"].includes(plan.reasoning?.verbosity) ? plan.reasoning.verbosity : "low";
  body.reasoning = { ...(body.reasoning && typeof body.reasoning === "object" ? body.reasoning : {}), effort };
  body.text = { ...(body.text && typeof body.text === "object" ? body.text : {}), verbosity };

  const minimumTokens = plan.model.tier === "sol" ? 1800 : plan.model.tier === "terra" ? 1250 : 800;
  body.max_output_tokens = Math.max(minimumTokens, Number(body.max_output_tokens) || 0);
  body.instructions = `${String(body.instructions || "")}\n\n${bellaCognitionInstructionV26(plan)}`;

  if (Array.isArray(body.tools) && plan.model.tier === "sol") {
    body.tools = body.tools.map(tool => tool?.type === "web_search"
      ? { ...tool, search_context_size: tool.search_context_size === "low" ? "medium" : tool.search_context_size }
      : tool);
  }
  return body;
}
...
function isModelAvailabilityError(response, data) {
  if (!response || response.ok) return false;
  const code = String(data?.error?.code || "").toLowerCase();
  const message = String(data?.error?.message || "").toLowerCase();
  return code.includes("model") || /model.*(?:not found|unsupported|access|available|exist)/.test(message);
}

function markModelFallbackV29(requestedModel, fallbackModel) {
  try {
    const context = getBellaRequestContextV22();
    const telemetry = context?.brainTelemetryV29;
    if (!telemetry || typeof telemetry !== "object") return;
    telemetry.fallbackUsed = true;
    telemetry.requestedModel = String(requestedModel || "").slice(0, 80);
    telemetry.fallbackModel = String(fallbackModel || "").slice(0, 80);
  } catch {}
}

async function fetchOpenAiWithFallback(input, init, body, plan) {
  let response = await nativeFetch(input, { ...init, body: JSON.stringify(body) });
  const fallback = String(plan?.model?.fallback || "").trim();
  if (response.ok || !fallback || fallback === body.model) return response;

  const data = await response.clone().json().catch(() => null);
  if (!isModelAvailabilityError(response, data)) return response;

  console.warn(`Bella v26 model ${body.model} unavailable; falling back to ${fallback}.`);
  markModelFallbackV29(body.model, fallback);
  const fallbackBody = { ...body, model: fallback };
...
function installOpenAiPersonaPatch() {
  ...
  globalThis.fetch = async function bellaV26Fetch(input, init = {}) {
    ...
    try {
      ...
      applyCognitiveBrainV26(body, context?.cognitivePlan);
      applySafeModeToOpenAi(body, resilience);
      return fetchOpenAiWithFallback(input, init, body, context?.cognitivePlan);
    } catch (error) {
      console.warn("Bella persona/experiment/cognition injection skipped:", error?.message || error);
      return nativeFetch(input, init);
    }
  };
}

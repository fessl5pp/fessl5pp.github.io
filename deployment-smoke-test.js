...
if (releaseHeader !== 'v12') fail('The production shell must expose Bella release v12.');

if (!app.includes('Bella v12 AI-first chat + coordinated ambient moments marker')) fail('app.js v12 release marker is missing.');
if (!app.includes('?v=16')) fail('Runtime modules must keep the validated v16 generation.');
if (!sw.includes('bella-pwa-v14-release-12')) fail('Service worker cache generation was not rotated for v12.');
...
if (!health.includes('release: "v12"')) fail('Deployment health endpoint must report release v12.');
...
console.log('Bella v12 deployment regression checks passed: build gate, static output, release headers, cache rotation and health endpoint are active.');

export default async function handler(req, res) {
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({ model: "gpt-5-mini", input: "رد فقط بكلمة: شغال", max_output_tokens: 80 })
    });
    const data = await response.json();
    if (!response.ok) return res.status(502).json({ ok: false, reason: data?.error?.code || data?.error?.message || "openai_error" });
    const parts = [];
